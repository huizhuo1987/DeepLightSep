...
class TwoLayerGanModel(BaseModel):
    def name(self):
        return 'TwoLayerGanModel'
    ...
    def initialize(self, opt):
        ...
        self.visual_names = ['rgb_img', 'im1', 'im2', 'chrom', 'predication', 'shading1', 'shading2']
    ...
    def backward_D(self):
        s1 = torch.mul(self.rgb_img, torch.div(self.shading1, self.shading1 + self.shading2 + 1e-6))
        s2 = torch.mul(self.rgb_img, torch.div(self.shading2, self.shading1 + self.shading2 + 1e-6))

        input_G_B1 = self.image_pool1.query(torch.cat((self.rgb_img, s1, s2), 1))
        pred_fake = self.netD(input_G_B1.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake, False)

        if self.L1Loss(self.shading1, self.im1, self.mask) < self.L1Loss(self.shading1, self.im2, self.mask):
            input_GT = torch.cat((self.rgb_img, self.img1, self.img2), 1)
        else:
            input_GT = torch.cat((self.rgb_img, self.img2, self.img1), 1)

        pred_real = self.netD(input_GT)
    ...
    def backward_G_B(self):
        ...
        s1 = torch.mul(self.rgb_img, torch.div(shading1, shading1 + shading2 + 1e-6))
        s2 = torch.mul(self.rgb_img, torch.div(shading2, shading1 + shading2 + 1e-6))
        pred_fake = self.netD(torch.cat((self.rgb_img, s1, s2), 1))
        ...
        self.loss_G_B = .5 * self.sloss(self.im1, self.im2, shading1, shading2, self.mask) \
                        ...
                        + .5 * self.criterionGAN(pred_fake, True)
